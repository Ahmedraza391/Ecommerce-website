...
if($_GET['message']=="update_order"){
    $query = mysqli_query($connection,"UPDATE order_notification SET status='seen'");
}
?>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="overflow w-100 " id="table_div">
                                        <table class="table text-dark table-bordered table-striped" id="old_table">
                                            ...
                                            <tbody id="tbody">
                                                <?php
                                                $query_table = "SELECT users.*,users.user_name as 'u_name',order_details.*, user_order.*, products.*,products.name as 'p_name' FROM order_details INNER JOIN user_order ON order_details.order_id = user_order.id INNER JOIN products ON order_details.product_id = products.id INNER JOIN users ON order_details.user_id = users.user_id WHERE user_order.order_status='pending' ORDER BY order_details.id DESC  LIMIT 10";                                                
                                                ...
                                                foreach ($result as $row) {
                                                    ...
                                                    echo "<td class='align-middle'><img src='$row[image]' width='150px' height='70px'/></td>";
                                                        ...
                                                        echo "<td class='align-middle'>$row[product_qty]</td>";
                                                        echo "<td class='align-middle'>$row[order_status]</td>";
                                                            ...
                                                            echo "<td class='align-middle'>
                                                                <a href='view_order.php?id=$row[order_id] & message=notification'>
                                                                    <button class='btn btn-success px-3 btn-sm '>
                                                                        View
                                                                    </button>
                                                                </a>
